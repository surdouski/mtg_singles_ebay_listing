...
def perform_card_detection(image, input_path, output_path, card_pool, hash_size=32, size_thresh=10000):
    ...
    for n, contour in enumerate(contours):
        ...
        card_label, card_name = add_card_to_detected_cards(card, detected_cards)

        draw_text_and_contours_image(card_label, contour, image_result, rectangle_points)
        draw_text_and_save_card_image(output_path, card_image, card_name, card_label, n)
...
def draw_text_and_save_card_image(output_path, card_image, card_name, card_label, n):
    cv2.putText(card_image, card_label, (0, 20),
                cv2.FONT_HERSHEY_SIMPLEX, 0.4, (143, 0, 255), 2)
    cv2.imwrite(f'{output_path}{card_name}_{n}.jpg', card_image)


def draw_text_and_contours_image(card_label, contour, image_result, rectangle_points):
    cv2.drawContours(image_result, [contour], -1, (0, 255, 0), 2)
    cv2.putText(image_result, card_label, _minumum_width_by_minimum_height(rectangle_points), cv2.FONT_HERSHEY_SIMPLEX,
        0.4, (143, 0, 255), 2)
